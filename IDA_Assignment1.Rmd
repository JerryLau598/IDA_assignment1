...
print(paste("The standard error of original data is", sd(databp$recovtime, na.rm = T))) #12.2092151699008
```
...
```{r,collapse=T}
print(paste("The mean of original data is", mean(databp$recovtime, na.rm = T))) # 19.27272
print(paste("The standard error of original data is", sd(databp$recovtime, na.rm = T))) # 12.2092
print(paste("The mean after mean imputation is", mean(recov_mi))) #19.27273
print(paste("The standard error after mean imputation is", sd(recov_mi))) #11.42068
```
...
```{r, collapse=T}
print(paste("The mean of original data is", mean(databp$recovtime, na.rm = T))) # 19.2727
print(paste("The standard error of original data is", sd(databp$recovtime, na.rm = T)))
print(paste("The mean after regression imputation is", mean(recov_ri))) #19.4442
print(paste("The standard error after regression imputation is", sd(recov_ri))) #11.5642
```
...
```{r}
cor(recov_ri, databp$logdose, use = "complete", method = "pearson") 
```
The Pearson correlations between the recovery
time after imputation and the blood pressure is given as 
```{r}
cor(recov_ri, databp$bloodp, use = "complete", method = "pearson") ##-0.01952862
```
...
```{r, collapse=T}
print(paste("The mean of original data is", mean(databp$recovtime, na.rm = T))) # 19.2727
print(paste("The standard error of original data is", sd(databp$recovtime, na.rm = T))) # 12.20921
print(paste("The mean after stochastic regression imputation is", mean(recov_sri))) # 18.9825
print(paste("The standard error after stochastic regression imputation is", sd(recov_sri))) # 12.1576
```
...
recov_ri[ind]
recov_obs = recov_ri[-ind]
recov_pre = recov_ri[ind]
```
...
idoner = numeric(n)
for (i in 1:n){
  idoner[i] = which.min((recov_obs-recov_pre[i])^2)
}
...
One of the potential problem is that when there are too many missing values or the pattern of observed values is almost identical, the standard error of the results would be extremely small, which lost the variability of the data.
